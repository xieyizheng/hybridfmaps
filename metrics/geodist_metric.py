...
def plot_pck(geo_err, threshold=0.20, steps=40):
    ...
    fig = plot_pck_multiple([geo_err], ['err'])
    return auc, fig, pcks
...
def plot_pcks(pcks, labels, title="title", threshold=0.20, steps=80):
    thresholds = np.linspace(0., threshold, steps)
    # display figure
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
...
@METRIC_REGISTRY.register()
def plot_pck_multiple(geo_errs, labels, title='title', threshold=0.20, steps=80):
